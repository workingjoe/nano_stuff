...
fpsReport = 0
scaleFactor = 3 # integer use 1:scaleFactor incoming image 
...
if (camSet == camSetUSB):
  cam = cv2.VideoCapture(camSet, cv2.CAP_V4L)
else:
  cam = cv2.VideoCapture(camSet, cv2.CAP_ANY)

# fixup USB camera to reasonable size and formatting
if (camSet == camSetUSB) and (cam.isOpened()) :
...
timeStamp = time.time() # grab initial time
while True:
    ret, frame = cam.read()

    if frame is None:
        print(f'No Frame was captured for some reason...stopping...')
        break

    # scale down version for comparison speed up, 
    frameSmall = cv2.resize(frame, (0,0), fx=(1/scaleFactor), fy=(1/scaleFactor) )
    ...
    # now lets round off some precision
    fps = (1 / deltaTime)
    # then use some low-pass filtering
    fpsReport=(0.80*fpsReport + 0.20*fps)

#    print(f'FPS is {round(fpsReport, 1)}')
    #make background rectangle for text
    cv2.rectangle(frame, (0,0), (100,30), (80,0,0),-1) # dark blue with YELLOW text below
    cv2.putText(frame, str(round(fpsReport, 1))+'fps', (0, 15), font, 0.75,(0,255,255),1 )
